...
但是却是以牺牲电路的带宽为代价，
这样的模拟放大器频率响应较差，无法放大较高频率信号，器件无法以高压摆率工作。
\par
对于静态低电压放大器来说，其供电电压一直加在晶体管的偏置回路上，也就是
无论有无信号输入，晶体管都需要持续地消耗电流以维持偏置点，这就导致了较高的静态功耗。
...
\end{equation}
可见提升$g_m$或提升级联后的$R_{out}$都能直接提高$A_{0,\mathrm{LVFIA}}$，从而减小闭环误差并提升有效积分增益。
...
相较传统固定偏置式电平移位，CLS更强调“误差相关性管理”，
尤其适合低电压动态放大器中“增益不足、线性下降、共模漂移敏感”的共性问题\cite{liu2020pingpongcls,li2024crcls}。
与通过提升电路跨导来提升增益的直接方式不同，CLS充分利用了开关电容
电路的特性，通过时钟控制的电荷重分配，通过改变开关电容之间的电压关系来实现电平移位，
从而提升动态放大器的增益和线性性能。
...
CLS的基本原理和思想可以概括为在基本动态放大器的输出端和整体的闭环放大器输出端之间
引入电平移位电容，以及一个受控的电平移位阶段，
...
从而提升了放大器的增益和线性性能；其二，电路整体输出电压不再直接是基本动态放大器的输出端
电压，更准确地，是基本动态放大器输出端电压与电平移位电容上电荷重分配引起的电压变化的叠加，
这一变化除了带来增益提升之外，还把有限开环增益和电源轨限制导致的增益误差映射为可被后级处理的相关误差，降低了非线性失真。
\par
