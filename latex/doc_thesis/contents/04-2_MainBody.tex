...
\subsection{浮动反相放大器 (Floating Inverter Amplifier, FIA)}
...
该结构在近年低功耗 模数转换器 (Analog-to-Digital Converter, ADC) 与 CDC 研究中被广泛采用
...
\subsection{摆幅增强型浮动反相放大器 (Swing-Enhanced Floating Inverter Amplifier, SEFIA)}

SEFIA（Swing-Enhanced Floating Inverter Amplifier）可视为对 FIA 的摆幅与线性增强，
其原理图和时序图如图\ref{fig:SEFIA}所示。
其主要思路是通过结构与时序协同，
扩大有效线性工作区并缓解大信号条件下的压缩失真。
在普通的浮动反相放大器FIA中，由于从VDD到VSS之间存在 p沟道金属氧化物半导体 (P-channel Metal–Oxide–Semiconductor, PMOS) 和 n沟道金属氧化物半导体 (N-channel Metal–Oxide–Semiconductor, NMOS) 堆叠，
...
是本论文重点探讨的课题。
\subsection{低压浮动反相放大器 (Low-Voltage Floating Inverter Amplifier, LVFIA)}
...
储能电容 (Reservoir Capacitor, CRES)，在初始化阶段时由电源对其充电，
放大阶段时电容放电偏置晶体管；同时对称的电路结构以及负载电容 (Load Capacitance, CL) 的设置
确保了电路不需要额外共模反馈，在复位阶段能自动稳定到共模电压 (common-mode voltage, VCM)。
具体改进的部分包含以下几点：其一，LVFIA在SEFIA的基础上，
去掉了浮空电容 (Floating Capacitor, CI) 并舍弃了推挽的电路结构，将每个晶体管的偏置回路物理分离，
例如由图1中可见给每个PMOS都设置了独立的VSS，给每个NMOS都设置了独立的VDD，
这样做最直接且重要的影响就是解除了偏置时PMOS和NMOS之间电压关系的耦合性，
注：文中符号说明——VDD (positive supply，电源正极)、VSS (negative supply/ground，电源负极)、VCM (common-mode voltage，共模电压)、VGS (gate–source voltage，栅源电压)、CC1/CC2 (timing capacitors，时序电容)、VBP/VBN (bias voltages，偏置电压)。
...
\subsubsection{开关电容 (Switched-Capacitor, SC) 放大器的基本结构}
...
在运放增益A有限的情况下，电路最终达到平衡后，放大器输入端的电压不再是虚地，而是存在
一个小的电压偏移$V_X$，这使得采样电容${C_1}$上存储的电荷无法完全转移到反馈电容${C_2}$上，
导致增益误差。
为了减小$V_X$的绝对值，提升放大器的开环增益A是提升闭环增益的一个直接方法，
但这通常会增加电路复杂度和功耗。
一个更好的想法是：不改变运放本身，而是通过外围电路设计和
...
\begin{figure}[htbp]
    ...
    \subfigure[CLS初步放大阶段时的电路连接图]{
            \includegraphics[width=0.8\textwidth]{doc_thesis/figs/04/CLS阶段1.png}}
    \hfill
    \subfigure[CLS电平移位阶段时的电路连接图]{
            \includegraphics[width=0.8\textwidth]{doc_thesis/figs/04/CLS阶段2.png}}
    \caption{CLS图示}
    \label{fig:CLS图示}
\end{figure}
...
在该阶段通过控制开关实现对输入信号的电平移位。此时电路包含六个时钟，
除了代表基本的复位阶段${\Phi_1}$和放大阶段${\Phi_2}$之外，
CLS电平移位相由时钟${\Phi_{2a}}$和${\Phi_{2b}}$控制，分别代表初步放大阶段和CLS电平移位阶段。
此时电路的时钟时序关系图如图\ref{fig:the_proposed_circuit_timing}所示。

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\textwidth]{doc_thesis/figs/04/时序图.png}
    \caption{\ptheadingfont{本论文所提出的基于LVFIA和CLS机制的动态放大器设计的时钟时序图}}
    \label{fig:the_proposed_circuit_timing}
\end{figure}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\textwidth]{doc_thesis/figs/04/CLS过程电平变化.png}
    \caption{\ptheadingfont{CLS过程中的电平变化，在$\Phi_{2b}$阶段LVFIA的
    输出电平$V_o$被电平移位操作拉低，提升了电荷转移效率，放大器整体输出$V_o(CLS)$提高}}
    \label{fig:CLS过程电平变化}
\end{figure}
...
则在LVFIA片外配置。令电容${C_{RES}}$和${C_c}$的值分别为25pF和5pF。
\par
...
在输入电压$V_{DD}$为0.6V（相应的共模电压$V_{CM}$为0.3V）、外界温度为25℃的条件下，
改变电容反馈比${\frac{C_1}{C_2} }$，测试应用了CLS前后的电路的等效增益，结果如表
\ref{tab:CLS电路等效开环增益}所示，可见应用了CLS技术之后，在SC放大器内的LVFIA等效
增益有了明显的提升，在$\frac{C_1}{C_2}$较低时，等效增益提升更为显著，约为16.3dB。
\begin{table}[htbp]
    \centering
    \caption{\ptheadingfont{不同C1/C2下的电路等效开环增益}}
    \label{tab:CLS电路等效开环增益}
    \begin{tabular}{|c|c|c|c|c|}
        \hline
         \diagbox{有无CLS}{等效增益/dB}{$\frac{C_1}{C_2}$} & 2 & 5 & 10 & 20  \\
        \hline
        有CLS & 44.3 & 41.6 & 39.3 & 37.1  \\
        \hline
        无CLS & 28.0 & 28.0  & 28.0 & 28.0  \\
        \hline
    \end{tabular}
\end{table}


表\ref{tab:CLS电路闭环增益}列出了在不同的$$\frac{C_1}{C_2}$$的条件下，
应用CLS前后SC放大器的闭环增益，并给出了其与理想增益的相对误差。
应用CLS后，电路的等效开环增益提升，其最主要的表现是整体SC放大器的闭环
增益更接近于理想的电容比值$\frac{C_1}{C_2}$，从而减小了增益误差。


\begin{table}[htbp]
    \centering
    \caption{\ptheadingfont{不同C1/C2下应用了CLS的SC放大器闭环增益与增益相对误差}}
    \label{tab:CLS电路闭环增益}
    \begin{tabular}{|c|c|c|c|c|c|}
        \hline
        \multirow{3}{*}{有CLS} & 理想增益${C_1/C_2}$& 2 & 5 & 10 & 20 \\
        \cline{2-6}
         ~ & 闭环增益 & 1.960 & 4.741 & 8.860 & 15.32  \\
        \cline{2-6}
         ~ & 相对误差 & 0.0400 & 0.0521  & 0.114 & 0.234  \\
        \hline
        \multirow{3}{*}{无CLS} & 理想增益${C_1/C_2}$& 2 & 5 & 10 & 20 \\
        \cline{2-6}
